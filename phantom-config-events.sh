...
curl -k -u "admin:${iid}" "https://127.0.0.1/rest/asset/7" -H 'Content-Type: application/json' -d "{\"ingest_now\":true,\"container_source_ids\":\"\",\"max_containers\":${NUMC},\"max_artifacts\":3}"
...
sleep 10
curl -k -u "admin:${iid}" "https://127.0.0.1/rest/system_settings" -H 'Content-Type: application/json' -d "{\"reindex_section\":true,\"reindex_select\":\"app\"}"
curl -k -u "admin:${iid}" "https://127.0.0.1/rest/system_settings" -H 'Content-Type: application/json' -d "{\"reindex_section\":true,\"reindex_select\":\"artifact\"}"
curl -k -u "admin:${iid}" "https://127.0.0.1/rest/system_settings" -H 'Content-Type: application/json' -d "{\"reindex_section\":true,\"reindex_select\":\"asset\"}"
curl -k -u "admin:${iid}" "https://127.0.0.1/rest/system_settings" -H 'Content-Type: application/json' -d "{\"reindex_section\":true,\"reindex_select\":\"container\"}"
...
curl -k -u "admin:${iid}" "https://127.0.0.1/rest/system_settings" -H 'Content-Type: application/json' -d "{\"reindex_section\":true,\"reindex_select\":\"actionrun\"}"
