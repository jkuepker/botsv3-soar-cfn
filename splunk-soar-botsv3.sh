...
wget "https://raw.githubusercontent.com/jkuepker/botsv3-soar-cfn/master/phantom_hec_inputs.conf"
wget "https://raw.githubusercontent.com/jkuepker/botsv3-soar-cfn/master/user-prefs.conf"
...
while [[ "$(curl -s -o /dev/null -w "%{http_code}" https://127.0.0.1:8089/services/authentication/users -k -u "admin:SPLUNK-$(curl -s http://instance-data/latest/meta-data/instance-id)")" != "200" ]]; do printf "Waiting for Splunk to become available\n"; sleep 10; done
iid=$(curl "http://instance-data/latest/meta-data/instance-id") su splunk -c '/opt/splunk/bin/splunk add user phantomsearchuser -password $iid -role phantomsearch -full-name "Phantom Search" -force-change-pass false -auth "admin:SPLUNK-$iid"'
iid=$(curl "http://instance-data/latest/meta-data/instance-id") su splunk -c '/opt/splunk/bin/splunk add user phantomdeleteuser -password $iid -role phantomdelete -full-name "Phantom Delete" -force-change-pass false -auth "admin:SPLUNK-$iid"'